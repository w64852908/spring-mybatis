...
　　ReentrantLock、CountDownLatch、Semaphore等类的实现都依赖AbstractQueuedSynchronzier。可以说它是理解Java锁的重要一环。  
　　它提供了一个基于FIFO的队列，可以用于构建锁或者其他相关同步装置的基础框架。AbstractQueuedSynchronizer使用一个int型的state属性来表示状态。子类通过继承并实现它的方法来管理其状态，管理的方式主要是通过acquire和release方法来操作状态。  
　　AbstractQueuedSynchronizer没有实现任何同步接口，它仅仅是定义了若干acquire之类的方法来供使用。该同步器即可以作为排他模式也可以作为共享模式，当它被定义为一个排他模式时，其他线程对其的获取就被阻止，而共享模式对于多个线程获取都可以成功。
